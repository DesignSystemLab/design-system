...
export const ModalPortal = (props: ModalPortalProps) => {
  ...
  const { id, isOpen, onClose, hasCloseIcon, disableOverlayClose } = useContext(ModalContext);
  ...
  return (
    <Portal className={createClassVariant('modal', 'portal')}>
      ...
      <div
        ...
        aria-modal
        className={createClassVariant('modal', 'content')}
        css={Style.portal(width, height)}
        {...rest}
      >
        {hasCloseIcon && (
