...
import type { StackProps, Spacing, SpacingMap, Direction, FlexToken, FlexMap } from './stackTypes';
...
export const useStyle = (stackProps: StackProps) => {
    ...
    const defaultSpacing: Spacing = spacing ?? 'md';
    ...
    const styleTheme = {
        defaultStackStyle() {
            return css({
                padding: '8px',
                display: 'flex'
                // gap: Object.keys(spacingMap).includes(defaultSpacing) ? `${spacingMap[defaultSpacing]}` : defaultSpacing
            });
        },
        ...
        setMarginStyle(spacingMap: SpacingMap, spacing: Spacing) {
            return {
                horizontal: css({
                    marginTop: 0,
                    marginRight: 0,
    ...
    const cloneElements = children?.map((element, index) => {
        ...
        if (isValidElement(element)) {
            return clone(element, {
                ...
                // css: index ? margin[defaultDirection] : margin['none']
            });
        }
    });
